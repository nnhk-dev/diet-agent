...
public class MealService {
    ...
    public List<MealItem> search(SourceMode sourceMode, Long userId, SlotBundle slots) {
        // A-7 个槽位维度各自转 JSON 数组字符串，空的维度 SQL 侧 '[]' = '[]' 自动跳过
        List<MealItemRow> rows = mealMapper.search(
                sourceMode,                                      // PERSONAL 或 PUBLIC
                userId,                                          // PERSONAL 时过滤 owner_user_id
                jsonService.toJsonArray(slots.mealTime()),       // 餐次 → JSON
                jsonService.toJsonArray(slots.mood()),           // 心情 → JSON
                jsonService.toJsonArray(slots.scene()),          // 场景 → JSON
                jsonService.toJsonArray(slots.healthGoal()),     // 健康目标 → JSON
                jsonService.toJsonArray(slots.cuisine()),        // 菜系 → JSON
                jsonService.toJsonArray(slots.taste()),          // 口味 → JSON
                jsonService.toJsonArray(slots.convenience()),    // 便捷性 → JSON
                SEARCH_LIMIT                                     // 最多 50 条
        );
        // B-Row → MealItem，交给 Rank 层精细排序
