...
public class MealRankService {
    ...
    public List<MealItem> rank(MealRankRequest request) {
        // A-排除已推荐过的 ID：用 HashSet 实现 O(1) 查找，"换一批"时不重复推
        ...
        return request.candidates().stream()
                .filter(item -> item != null && !excludeIds.contains(item.id()))  // 过滤 null 和已推过的
                .map(item -> withRankScore(item, request.slots()))                // B-逐条计算匹配分
                .sorted(Comparator.comparingDouble((MealItem item) -> item.matchScore()).reversed()) // C-按分数降序
                .limit(10)   // D-取 top10 给 RecommendResponseAgent
                .toList();
    }
